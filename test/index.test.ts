...
import { synapse, safesynapse } from "../src";
...
describe("synapse", () => {
  it("returns the passed value if it's not a string", () => {
    ...
    for (const testCase of testCases) {
      expect(synapse(testCase.input)).toStrictEqual(testCase.input);
    }
  });

  it("parses string 'true' as boolean `true` case-insensitively", () => {
    expect(synapse("true")).toStrictEqual(true);
    expect(synapse("TRUE")).toStrictEqual(true);
  });

  it("parses string 'false' as boolean `false` case-insensitively", () => {
    expect(synapse("false")).toStrictEqual(false);
    expect(synapse("FALSE")).toStrictEqual(false);
  });

  it("parses string 'null' as `null`", () => {
    /* eslint-disable unicorn/no-null */
    expect(synapse("null")).toBeNull();
    expect(synapse("NULL")).toBeNull();
    /* eslint-enable unicorn/no-null */
  });

  it("parses string 'NaN' as `Number.NaN` case-insensitively", () => {
    expect(synapse("nan")).toBeNaN();
    expect(synapse("Nan")).toBeNaN();
  });

  it("parses string 'infinity' as `Number.POSITIVE_INFINITY` case-insensitively", () => {
    expect(synapse("infinity")).toStrictEqual(Number.POSITIVE_INFINITY);
    expect(synapse("Infinity")).toStrictEqual(Number.POSITIVE_INFINITY);
  });

  it("parses string '-infinity' as `Number.NEGATIVE_INFINITY` case-insensitively", () => {
    expect(synapse("-infinity")).toStrictEqual(Number.NEGATIVE_INFINITY);
    expect(synapse("-Infinity")).toStrictEqual(Number.NEGATIVE_INFINITY);
  });

  it("parses string 'undefined' as `undefined` case-insensitively", () => {
    expect(synapse("undefined")).toBeUndefined();
    expect(synapse("UNDEFINED")).toBeUndefined();
  });

  it("parses with surrounding spaces", () => {
    expect(synapse("  true ")).toBe(true);
    expect(synapse(" -123 ")).toStrictEqual(-123);
    expect(synapse(' { "test": 123 }  ')).toStrictEqual({ test: 123 });
  });
  ...
  it("parses valid JSON texts", () => {
    ...
    for (const testCase of testCases) {
      expect(synapse(testCase.input)).toStrictEqual(testCase.output);
    }
  });
  ...
  it("prevents prototype pollution", () => {
    ...
    const warnMessage = (key: string) =>
      `[synapse] Dropping "${key}" key to prevent prototype pollution.`;
  ...
  describe("throws an error if it's a invalid JSON texts with safesynapse", () => {
    const testCases = [
    ...
    for (const testCase of testCases) {
      it(testCase.input, () => {
        expect(() => safesynapse(testCase.input)).toThrowError(
          testCase.output || "",
        );
      });
    }
  ...
  it("throws an error on possible prototype pollution with safesynapse", () => {
    const testCases = [
      {
        input: '{ "__proto__": {} }',
        output: {},
    ...
    for (const testCase of testCases) {
      expect(() => safesynapse(testCase.input)).toThrowError(
        "[synapse] Possible prototype pollution",
      );
    }
  });
